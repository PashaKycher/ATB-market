...
const EditSubCategory = ({ close, subCategoryData, closeView }) => {
  ...
  const hendleRemoveCategorySelecter = (categoryId) => {
    const index = uploadSubCategoryData.category.findIndex((item) => item._id === categoryId);
    const newSubCategory = [...uploadSubCategoryData.category];
        ...
        <form className='my-3 grid gap-3' onSubmit={hendleOnSubmit}>
          ...
          <div className='grid gap-2'>
            {/* display valey category */}
            {
              uploadSubCategoryData.category[0] ? (
                <div className='flex gap-2 flex-wrap text-sm text-neutral-600 border rounded p-2
                  ...
                  {
                    uploadSubCategoryData.category.map((item, index) => {
                      return (
                        <div key={index + "SubCategoryValey"} className='shadow-md flex'>
                          {item?.name}<span> </span>
                          <div
                            className='hover:bg-red-600 hover:text-white hover:border-red-600 cursor-pointer
                                        bg-red-100 p-1 rounded-full border border-red-100'
            ...
            <select
              ...
              name='category'
              value={uploadSubCategoryData.category}
              className='border border-blue-100 bg-blue-50 rounded w-full p-2
                                        outline-none focus:border-primary-200'
              onChange={(e) => {
          ...
          <div className='grid gap-2'>
            ...
            <div className='flex gap-4 flex-col md:flex-row items-center'>
              ...
              <label htmlFor='subCategoryImage'>
                <div
                  disabled={!uploadSubCategoryData.name || !uploadSubCategoryData.category}
                  className={`px-4 py-2 rounded border font-medium
                                                ${!uploadSubCategoryData.name ? "bg-gray-400 cursor-not-allowed" :
          ...
          <button
            disabled={!uploadSubCategoryData.name || !uploadSubCategoryData.image || !uploadSubCategoryData.category[0]}
            className={`py-2 rounded w-full font-semibold
                                    ${!uploadSubCategoryData.name || !uploadSubCategoryData.image || !uploadSubCategoryData.category[0] ?
